...
@patch('Robot.cycle.cycle.TCPClient')
@patch('Robot.cycle.cycle.config.Config')
@patch('Robot.cycle.cycle.Country')
@patch('Robot.cycle.cycle.Cube')
...
class CycleTest(unittest.TestCase):
    ...
    def _setup_cube_mock(mock, position, target_zone_position):
        ...
        cube_mock.get_target_zone_position = Mock(return_value=target_zone_position)
        mock.return_value = cube_mock

    @staticmethod
    def _setup_tcp_client_mock():
        tcp_client_mock = MagicMock()
        tcp_client_mock.connect_socket = Mock()
        tcp_client_mock.send_data = Mock()
        tcp_client_mock.disconnect_socket = Mock()
        return tcp_client_mock

    def test_when_start_cycle_then_move_to_atlas_is_called(self, RobotControllerMock, CubeMock,
    ...
    def test_continue_cycle_with_instruction_remaining_then_next_instruction_is_called(self, RobotControllerMock,
                                                                                       CubeMock, CountryMock, ConfigMock,
                                                                                       TCPClientMock):
        self._setup_config_mock(ConfigMock)
        robot_controller_mock = MagicMock()
    ...
    def test_continue_cycle_when_robot_arrived_at_atlas_zone_then_multiple_functions_are_called(self, QuestionMock, CountryRepoMock,
        ...
        robot_controller_mock.robot_is_next_to_target_with_correct_orientation = Mock(return_value=False)
    ...
    def test_continue_cycle_when_robot_arrived_at_cube_then_push_cube_is_called(self, RobotControllerMock, CubeMock,
                                                                                CountryMock, ConfigMock, TCPClientMock):
        self._setup_config_mock(ConfigMock)
        self._setup_cube_mock(CubeMock, None, None)
        robot_controller_mock = MagicMock()
        robot_controller_mock.instruction_remaining = Mock(return_value=False)
        robot_controller_mock.robot_is_next_to_target_with_correct_orientation = Mock(return_value=True)
