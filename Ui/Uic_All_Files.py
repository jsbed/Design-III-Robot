...
def uicFile(folder_fullpath, filename):
    ...
    output_fullpath = os.path.join(folder_fullpath, "GeneratedFiles",
...
def qrcFile(folder_fullpath, filename):
    ...
    input_fullpath = os.path.join(folder_fullpath, filename)
    output_fullpath = os.path.join(folder_fullpath, "GeneratedFiles",
                                   os.path.splitext(filename)[0] + "_rc.py")

    # Execute rcc tool
    print("  -  " + input_fullpath)
    subprocess.call([PysideRCC, "-py3", input_fullpath, "-o", output_fullpath])
