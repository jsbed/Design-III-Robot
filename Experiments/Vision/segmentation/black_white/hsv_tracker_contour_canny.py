...
while True:
    ...
    # (find contours)
    lower_thresh = cv2.getTrackbarPos('lower-thresh','dst')
    higher_thresh = cv2.getTrackbarPos('higher-thresh','dst')
    #print(lower_thresh)
    #print(higher_thresh)

    gray = cv2.cvtColor(res, cv2.COLOR_BGR2GRAY)
    ret,thresh = cv2.threshold(gray,lower_thresh,higher_thresh, 0)
    contours = cv2.findContours(thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    new = img_bgr.copy()
    cv2.drawContours(new, contours[1], -1, (0,255,0), 3)    
    ...
    if cc == 1048586: # Touche Enter (save data)
        with open(os.path.join(DATA_DIRECTORY, "data_{}.txt".format(str(DATA_COUNT))), "w") as file:
            ...
            file.write(", ".join(map(str, [lower_thresh, higher_thresh])))
